    ...
    <main class="app-content">
      <div class="app-title">
        ...
        <ul class="app-breadcrumb breadcrumb side">
          ...
          <li class="breadcrumb-item active">Consultant Lists</li>
        </ul>
       <button type="button" style="margin-top:10px; float: right; margin-left: 100px; margin-bottom:-15px;" class="bg-info text-white"><i class="fa fa-plus-circle" aria-hidden="true"><a href="addConsultants" style="text-decoration:none; color:white;"> Add Consultant</a></i></button>
      </div>
      </div>
      <div class="row">
      <c:if test="${resultFlag==0}">
         ...
         <c:if test="${resultFlag==1}">
         	<div class="col-md-12">
			  <div class="alert alert-success alert-dismissible fade show" role="alert" style="text-align:right; font-weight:bold; font-size:bold;">
                   ${message}
                    ...
                    <% int index=1; %>
						<c:forEach var="consultant" items="${list}">
							<tr>
								...
								<td width="15%">${consultant.dutyDateFrom} - ${consultant.dutyDateTo }</td>
								<td width="10%">${consultant.dutyTimeFrom} - ${consultant.dutyTimeTo }</td>
								<td width="12%">${consultant.specialist }</td>
								<td>${consultant.email }</td>
								<td>${consultant.phone }</td>
								<td width="20%">${consultant.degree }</td>
								<td><a href="editConsultants/${consultant.consultantId}" class="settings"
